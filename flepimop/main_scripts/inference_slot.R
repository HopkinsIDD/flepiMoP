...
for(seir_modifiers_scenario in seir_modifiers_scenarios) {
  ...
  for(outcome_modifiers_scenario in outcome_modifiers_scenarios) {
    ...
    initial_spar <- read_parquet_with_check(first_chimeric_files[['spar_filename']])
    initial_hpar <- read_parquet_with_check(first_chimeric_files[['hpar_filename']])
    initial_snpi <- read_parquet_with_check(first_chimeric_files[['snpi_filename']])
    initial_hnpi <- read_parquet_with_check(first_chimeric_files[['hnpi_filename']])
    if (!is.null(config$initial_conditions)){
      initial_init <- read_parquet_with_check(first_chimeric_files[['init_filename']])
    }
    if (!is.null(config$seeding)){
    ...
    chimeric_current_likelihood_data <- read_parquet_with_check(first_chimeric_files[['llik_filename']])
    global_current_likelihood_data <- read_parquet_with_check(first_global_files[['llik_filename']]) # they are the same ... don't need to load both
