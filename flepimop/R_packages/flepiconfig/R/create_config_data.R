...
#' Generate parameters for a modifier (i.e., an intervention/npi)
#'
...
#' dat
#'
set_npi_params <- function(npi_name = "waning_npi",
                           param_name = "epsilon",
    ...
    local_var <- dplyr::tibble(USPS = "",
                               ...
                               modifier_method = modifier_method,
                               value_dist = v_dist,
                               value_mean = v_mean,
                               value_sd = v_sd,
                               value_a = v_a,
...
#' Specify parameters for NPIs from file
#'
#' @param intervention_file df with the location's state and ID and the intervention start and end dates, name, and modifier_method - from process_npi_shub
#' @param param_name name of parameter NPIs are modifying
#' @param sim_start_date simulation start date
#' @param sim_end_date simulation end date
#' @param npi_cutoff_date only interventions that start before or on npi_cuttof_date are included
#' @param redux_subpops string or vector of characters indicating which subpop will have an intervention with the ModifierModifier modifier_method; it accepts "all". If any values are specified, the intervention in the subpop with the maximum start date will be selected. It defaults to NULL. .
#' @param v_dist type of distribution for reduction
#' @param v_mean reduction mean
#' @param v_sd reduction sd
#' @param v_a reduction a
...
#'
set_npi_params_fromfile <- function(intervention_file, 
                                    param_name = "r0",
    ...
    npi_cutoff_date <- lubridate::ymd(npi_cutoff_date)
    
    npi_cutoff_date <- min(npi_cutoff_date, sim_end_date)
    
    npi <- intervention_file %>% 
        dplyr::filter(start_date <= npi_cutoff_date) %>% 
        dplyr::mutate(end_date = dplyr::case_when(is.na(end_date) ~ npi_cutoff_date, TRUE ~ end_date)) %>%
        dplyr::filter((end_date >= sim_start_date | is.na(end_date)) & (start_date < npi_cutoff_date)) %>% 
        # dplyr::group_by(USPS, subpop) %>% 
        dplyr::mutate(end_date = dplyr::case_when(is.na(end_date) | end_date > npi_cutoff_date ~ npi_cutoff_date, TRUE ~ end_date)) %>%
        dplyr::mutate(start_date = dplyr::case_when(is.na(start_date) | start_date < sim_start_date ~ sim_start_date, TRUE ~ start_date)) %>%
        dplyr::mutate(value_dist = v_dist, 
                      ...
                      category = "NPI", baseline_scenario = "", 
                      parameter = dplyr::if_else(modifier_method == "MultiPeriodModifier", param_name, NA_character_))
    ...
    npi <- npi %>% dplyr::mutate(dplyr::across(pert_mean:pert_b, ~ifelse(inference, .x, NA_real_)), pert_dist = ifelse(inference, pert_dist, NA_character_)) %>% 
        dplyr::select(USPS, subpop, 
                      start_date, end_date, name, modifier_method, type, category, 
                      parameter, baseline_scenario, tidyselect::starts_with("value_"), 
                      tidyselect::starts_with("pert_"))
    if (!is.null(redux_subpops)) {
        if (redux_subpops == "all") {
            redux_subpops <- unique(npi$subpop)
        }
        npi <- npi %>% dplyr::filter(subpop %in% redux_subpops) %>% 
            dplyr::group_by(subpop) %>% dplyr::filter(start_date == max(start_date)) %>%
            dplyr::mutate(category = "base_npi", name = paste0(name, "_last")) %>% 
            dplyr::bind_rows(npi %>% dplyr::group_by(subpop) %>% dplyr::filter(start_date != max(start_date) | !subpop %in% redux_subpops)) %>% 
            dplyr::ungroup()
    }
    npi <- npi %>% dplyr::ungroup() %>% 
        dplyr::add_count(name) %>% 
        dplyr::mutate(modifier_method = dplyr::if_else(n == 1 & modifier_method == "MultiPeriodModifier", "SinglePeriodModifier", modifier_method), 
                      parameter = dplyr::if_else(n == 1 & modifier_method == "SinglePeriodModifier", param_name, parameter)) %>% 
        dplyr::select(-n)
    return(npi)
}
...
set_seasonality_params <- function(param_name = "r0",
                                   sim_start_date=as.Date("2020-03-31"),
                                   sim_end_date=Sys.Date()+60,
                                   inference = TRUE,
                                   modifier_method = "MultiPeriodModifier",
                                   v_dist="truncnorm",
                                   v_mean = c(1.2, 1.133, 1.067, 1, 1-0.067, 1-0.133, 1-0.2, 1-0.133, 1-0.067, 1, 1.067, 1.133),
                                   v_sd = 0.05, v_a = 0, v_b = 3,
                                   p_dist="truncnorm",
                                   p_mean = 0, p_sd = 0.05, p_a = -1, p_b = 1){
    ...
    seas <- tidyr::expand_grid(
        ...
        dplyr::add_count(name) %>%
        dplyr::mutate(modifier_method = dplyr::if_else(n > 1, modifier_method, "SinglePeriodModifier"),
                      end_date = dplyr::if_else(end_date > sim_end_date, sim_end_date, end_date),
                      start_date = dplyr::if_else(start_date < sim_start_date, sim_start_date, start_date)
        ) %>%
        dplyr::select(USPS, subpop, start_date, end_date, name, modifier_method, type, category, parameter, baseline_modifier, tidyselect::starts_with("value_"), tidyselect::starts_with("pert_"))
...
#' dat
#'
set_localvar_params <- function(param_name = "r0",
                                sim_start_date=as.Date("2020-03-31"),
                                sim_end_date=Sys.Date()+60,
                                inference = TRUE,
                                v_dist="truncnorm",
...
set_redux_params <- function(npi_file,
    ...
    redux <- dplyr::tibble(
        ...
        mutate(USPS = "",
               ...
               modifier_method = "ModifierModifier",
               parameter = param_name,
               value_dist = v_dist,
               value_sd = v_sd,
               value_a = v_a,
...
set_vacc_rates_params <- function (vacc_path,
    ...
    vacc <- vacc %>% dplyr::filter(start_date <= sim_end_date) %>%
        ...
        dplyr::mutate(subpop = as.character(subpop), month = lubridate::month(start_date, label = TRUE),
                      ...
                      modifier_method = "SinglePeriodModifier",  baseline_modifier = "",
    ...
    vacc <- vacc %>%
        dplyr::select(USPS, subpop, start_date, end_date, name,
                      modifier_method, type, category, parameter, baseline_modifier,
                      tidyselect::starts_with("value_"), tidyselect::starts_with("pert_")) %>%
    ...
    variant_data <- variant_data %>% dplyr::mutate(type = "transmission",
        ...
        dplyr::mutate(dplyr::across(pert_mean:pert_b, ~ifelse(inference & start_date < inference_cutoff_date, .x, NA_real_)),
                      pert_dist = ifelse(inference & start_date < inference_cutoff_date,
                                         pert_dist, NA_character_)) %>%
        dplyr::select(USPS,
                      subpop, start_date, end_date, name, modifier_method, type, category,
...
set_vacc_outcome_params <- function(age_strat = "under65",
    ...
    outcome <- outcome %>%
        ...
        dplyr::mutate(
            #    name = paste(param, "vaccadj", month, sep = "_"), modifier_method = "SinglePeriodModifier",
            #    name = paste(param, "vaccadj", USPS, (1-value_mean), sep = "_"), modifier_method = "SinglePeriodModifier",
            name = paste(param, "vaccadj", (1-value_mean), sep = "_"), modifier_method = "SinglePeriodModifier",
...
)
{
    ...
    outcome <- outcome %>% dplyr::mutate(param = "incidH") %>%
        ...
        dplyr::mutate(dplyr::across(pert_mean:pert_b, ~ifelse(inference, .x, NA_real_)),
                      pert_dist = ifelse(inference, pert_dist, NA_character_)) %>%
        dplyr::select(USPS, subpop, start_date, end_date, name, modifier_method, type, category,
                      parameter, baseline_modifier, tidyselect::starts_with("value_"),
                      tidyselect::starts_with("pert_"))
