...
def record_click_help(
    click_object: click.Command | click.Group, command_info: dict
) -> dict:
    """
    ...
    if isinstance(click_object, (click.Command, click.Group)):
        ...
        raise TypeError(
            f"Error, click_object must be either click.Group or click.Command: {type(click_object)}"
        )
    ...
    if isinstance(click_object, click.Group):
        for _, subcommand in click_object.commands.items():
            ...
            command_info[click_object.name][subcommand.name] = subcommand_dict[
                subcommand.name
            ]
...
def output_help_table(help_dict: dict) -> dict[str:str]:
    ...
    if depth == 4:
        for command in help_dict.keys():
            for subcommand in help_dict[command].keys():
                ...
                markdown_table = tabulate(
                    data_structure, headers="firstrow", tablefmt="pipe"
                )
        ...
        raise Exception(
            f"help_dict has depth of {depth}, meaning there is more than one layer of subcommands (no framework to support)"
        )  # expand to make more dynamic
...
def append_md_file(help_dict: dict[str : list[dict, dict]]) -> None:
    ...
    file_path = os.path.abspath(
        os.path.join(
            ...
            "../../../documentation/gitbook/gempyor/model-implementation/flepimop-click-commands.md",
        )
    )
    ...
    if start_index is not None:
        new_file_content = lines[
            : start_index + 1
        ]  # Keep content before start point, append after
...
def main():
    ...
    for command in cli.commands:
        flepimop_commands[cli.commands[command].name] = [
            record_click_help(cli.commands[command], {})
        ]
