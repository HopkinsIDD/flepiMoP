...
@pytest.mark.parametrize(
    "filters", [[], ["- /foo/bar.txt"], ["+ /fizz/buzz.txt"], ["/a/b/c.txt"]]
)
@pytest.mark.parametrize(
    "overrides",
    [
...
def test_list_filters(
    ...
    any_minus = (
        any(f.startswith("- ") for f in (filters if overrides is None else overrides))
        or any(f.startswith("- ") for f in (prefix or []))
        or any(f.startswith("- ") for f in (suffix or []))
    )
    parsed_count = sum(
        (
    ...
    assert len(parsed_filters) == parsed_count + int(not any_minus) * min(parsed_count, 1)
    if reverse:
