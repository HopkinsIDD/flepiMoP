...
def test_full_npis_read_write():
    ...
    inference_simulator.modinf.write_simID(
...
def test_spatial_groups():
    ...
    npi = seir.build_npi_SEIR(
        inference_simulator.modinf, load_ID=False, sim_id2load=None, config=config
    )

    # all independent: r1
    assert (
        len(npi.getReduction("r1")["2021-01-01"].unique())
        == inference_simulator.modinf.nsubpops
    )
    ...
    # two groups: r3
    assert (
        len(npi.getReduction("r3")["2020-04-15"].unique())
        == inference_simulator.modinf.nsubpops - 2
    )
    assert npi.getReduction("r3").isna().sum().sum() == 0
    assert (
        len(npi.getReduction("r3").loc[["01000", "02000"], "2020-04-15"].unique()) == 1
    )
    assert (
        len(npi.getReduction("r3").loc[["04000", "06000"], "2020-04-15"].unique()) == 1
    )
    ...
    assert (
        len(npi.getReduction("r4").loc[["01000", "02000"], "2020-04-15"].unique()) == 1
    )
    assert (
        len(npi.getReduction("r4").loc[["04000", "06000"], "2020-04-15"].unique()) == 2
    )
    ...
    assert (
        len(npi.getReduction("r5").loc[["01000", "04000"], "2020-10-15"].unique()) == 1
    )
    assert (
        len(npi.getReduction("r5").loc[["02000", "06000"], "2020-10-15"].unique()) == 2
    )
    ...
    assert len(df) == 1
    assert set(df["subpop"].iloc[0].split(",")) == set(
        inference_simulator.modinf.subpop_struct.subpop_names
    )
    ...
    assert (
        df[df["subpop"] == "09000,10000"]["start_date"].iloc[0]
...
def test_spatial_groups():
    ...
    snpi_read = pq.read_table(
        f"{config_filepath_prefix}model_output/snpi/000000001.105.snpi.parquet"
    ).to_pandas()
    snpi_read["value"] = np.random.random(len(snpi_read)) * 2 - 1
    out_snpi = pa.Table.from_pandas(snpi_read, preserve_index=False)
    pa.parquet.write_table(
        out_snpi, file_paths.create_file_name(106, "", 1, "snpi", "parquet")
    )
    ...
    inference_simulator.modinf.write_simID(
        ftype="snpi", sim_id=1, df=npi_seir.getReductionDF()
    )

    snpi_read = pq.read_table(
        f"{config_filepath_prefix}model_output/snpi/000000001.106.snpi.parquet"
    ).to_pandas()
    snpi_wrote = pq.read_table(
        f"{config_filepath_prefix}model_output/snpi/000000001.107.snpi.parquet"
    ).to_pandas()

    # now the order can change, so we need to sort by subpop and start_date
    snpi_wrote = snpi_wrote.sort_values(by=["subpop", "start_date"]).reset_index(
        drop=True
    )
    snpi_read = snpi_read.sort_values(by=["subpop", "start_date"]).reset_index(
        drop=True
    )
    assert (snpi_read == snpi_wrote).all().all()

    npi_read = seir.build_npi_SEIR(
        inference_simulator.modinf,
        load_ID=False,
        sim_id2load=1,
        config=config,
        bypass_DF=snpi_read,
