    ...
    for i, place in enumerate(subpop):
        for dt in hosp.index:
            if dt.date() == date_data:
                ...
                assert (
                    hosp[hosp["subpop"] == place]["incidICU"][
                ...
                assert (
                    hosp[hosp["subpop"] == place]["incidI"][dt - datetime.timedelta(7)]
                    == 0
                )
                assert (
                    hosp[hosp["subpop"] == place]["incidD"][dt - datetime.timedelta(4)]
                    == 0
                )
...
def test_outcome_modifiers_scenario_with_load():
    ...
    for out in ["incidH", "incidD", "incidICU"]:
        for i, place in enumerate(subpop):
            ...
            b = hpar_config[
                (hpar_rel["outcome"] == out) & (hpar_config["subpop"] == place)
            ]
            assert len(a) == len(b)
...
def test_outcomes_npi():
    ...
    outcomes.onerun_delayframe_outcomes(
        sim_id2write=1, modinf=inference_simulator.modinf
    )
