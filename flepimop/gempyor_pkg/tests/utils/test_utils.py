...
def test_read_df_fail(fname, extension):
    with pytest.raises(NotImplementedError, match=rf"Invalid extension provided: '{extension}'. Supported extensions are `.csv` or `.parquet`."):
        os.chdir(tmp_path)
...
def test_get_filetype_resume_discard_seeding_true_flepi_block_index_1():
    ...
    assert (
        utils.get_filetype_for_resume(resume_discard_seeding="true", flepi_block_index="1")
        == expected_types
    )
...
def test_get_filetype_resume_discard_seeding_false_flepi_block_index_1():
    ...
    assert (
        utils.get_filetype_for_resume(resume_discard_seeding="false", flepi_block_index="1")
        == expected_types
    )
...
def test_get_filetype_flepi_block_index_2():
    ...
    assert (
        utils.get_filetype_for_resume(resume_discard_seeding="false", flepi_block_index="2")
        == expected_types
    )
