...
class MultiTimeReduce(NPIBase):
    def __init__(
        ...
        *,
        ...
        self.parameters = pd.DataFrame(
            data={
                "npi_name": [""] * len(self.subpop),
                "parameter": [""] * len(self.subpop),
                "start_date": [[self.start_date]] * len(self.subpop),
                "end_date": [[self.end_date]] * len(self.subpop),
                "reduction": [0.0] * len(self.subpop),
            },
            index=self.subpop,
        )
        ...
        for grp_config in npi_config["groups"]:
            subpop_grp = self.__get_subpop_grp(grp_config)
            for sub_index in range(len(self.parameters["start_date"][subpop_grp[0]])):
                period_range = pd.date_range(
                    self.parameters["start_date"][subpop_grp[0]][sub_index],
                    self.parameters["end_date"][subpop_grp[0]][sub_index],
                )
                self.npi.loc[subpop_grp, period_range] = np.tile(
                    self.parameters["reduction"][subpop_grp],
                    (len(period_range), 1),
                ).T
    ...
    def __checkErrors(self):
        ...
        for n in self.subpop:
            if n not in self.subpop:
                raise ValueError(f"Invalid config value {n} not in subpop")
    ...
    def __createFromConfig(self, npi_config):
        ...
        self.subpop = self.__get_subpop(npi_config)

        self.parameters = self.parameters[self.parameters.index.isin(self.subpop)]
        ...
        for grp_config in npi_config["groups"]:
            ...
            this_spatial_group = helpers.get_spatial_groups(grp_config, subpop_grp)
    ...
    def __get_subpop_grp(self, grp_config):
        if grp_config["subpop"].get() == "all":
            subpop_grp = self.subpop
        else:
            subpop_grp = [str(n.get()) for n in grp_config["subpop"]]
        return subpop_grp
    ...
    def __createFromDf(self, loaded_df, npi_config):
        ...
        # self.subpop = set(self.parameters.index)

        if self.sanitize:
            if len(self.subpop) != len(self.parameters):
                print(f"loading {self.name} and we got {len(self.parameters)} subpop")
                print(f"getting from config that it affects {len(self.subpop)}")
        ...
        self.parameters = self.parameters.loc[list(self.subpop)]
        # self.parameters = self.parameters[self.parameters.index.isin(self.subpop) ]
        # self.parameters = self.parameters[self.subpop]
    ...
    def __get_subpop(self, npi_config):
        # Optional config field "subpop"
        # If values of "subpop" is "all" or unspecified, run on all subpop.
        # Otherwise, run only on subpop specified.
        subpop_grp = []
        for grp_config in npi_config["groups"]:
            if grp_config["subpop"].get() == "all":
                subpop_grp = self.subpop
                ...
                subpop_grp += [str(n.get()) for n in grp_config["subpop"]]
        subpop = set(subpop_grp)
        if len(subpop) != len(subpop_grp):
            raise ValueError(f"In NPI {self.name}, some subpop belong to several groups. This is unsupported.")
        return subpop
    ...
    def getReductionToWrite(self):
        df_list = []
        # self.parameters.index is a list of subpop
