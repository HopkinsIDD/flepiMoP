...
def config_plot(config_filepath, project_path, run_id, nsamples, subpop):
    ...
    if (
        ...
        npi_seir = gempyor.seir.build_npi_SEIR(
            modinf=gempyor_inference.modinf, load_ID=False, sim_id2load=None, config=config
        )
    if (
        gempyor_inference.modinf.outcomes_config is not None
        and gempyor_inference.modinf.npi_config_outcomes
    ):
    ...
    if subpop is None:
        ...
        subpop = [subpop]
    for sp in subpop:
        plot_modifers_activation(
            npi_seir=npi_seir, filename=f"seir_modifiers_activation_{sp}.pdf", subpop=sp
        )
        plot_modifers_activation(
            npi_seir=npi_outcomes,
    ...
    for sample in range(nsamples):
        if True:
            ...
            p_draw = gempyor_inference.get_seir_parameters(
                bypass_FN=fn.replace("snpi", "spar"), load_ID=True
            )
            npi_seir = gempyor.seir.build_npi_SEIR(
                modinf=gempyor_inference.modinf,
                load_ID=True,
    ...
    plot_npi_timeseries(
        npi_outcomes,
        gempyor_inference,
        filename=f"outcomesNPIcaveat.pdf",
        nsamples=nsamples,
    )
...
def plot_modifers_activation(npi_seir, filename: str, subpop: str):
    ...
    with PdfPages(filename) as pdf:
        ...
        for param in npi_seir.getReductionDF()["parameter"].unique():
            ...
            data = data[
                data.apply(lambda row: row.astype(str).str.contains(subpop).any(), axis=1)
            ]
    ...
    for k, uniq_pname in tqdm.tqdm(
        ...
        if True:  # we need to add filtering and prine here.
            fig, axes = plt.subplots(
                len(gempyor_inference.modinf.subpop_struct.subpop_names),
                1,
            ...
            for i, geoid in enumerate(gempyor_inference.modinf.subpop_struct.subpop_names):
                ...
                for l in range(len(all_parsed_params)):
                    df = pd.DataFrame(
                        all_parsed_params[l][k, :, i],
                        index=pd.date_range(
                            gempyor_inference.modinf.ti,
                            gempyor_inference.modinf.tf,
                            freq="D",
                        ),
                    )
                    ax.plot(df, lw=0.5)  # df[:'2021-12-31']
                fig.autofmt_xdate()
            fig.tight_layout()
            pdf.savefig(fig)
            plt.close(fig)
        ...
        for i, geoid in enumerate(gempyor_inference.modinf.subpop_struct.subpop_names):
            ...
            for sample in range(nsamples):
                ...
                df = pd.DataFrame(
                    rd[:, i],
                    index=pd.date_range(
                        gempyor_inference.modinf.ti, gempyor_inference.modinf.tf, freq="D"
                    ),
                )
                ax.plot(df, lw=0.5)  # df[:'2021-12-31']
            fig.autofmt_xdate()
