...
def rk4_integration(
    *,
    ...
    def rhs(t, x, today):
        ...
        if (x < 0).any():
            print(
                "Integration error: rhs got a negative x (pos, time)",
                np.where(x < 0),
                t,
            )
                ...
                if first_proportion:
                    ...
                    for spatial_node in range(nspatial_nodes):
                        proportion_keep_compartment = (
                            1 - percent_day_away * proportion_who_move[spatial_node]
                        )
                        ...
                        rate_change_compartment = proportion_change_compartment * (
                            relevant_number_in_comp[visiting_subpop]
                            ** relevant_exponent[visiting_subpop]
                        )
                        rate_change_compartment /= population[visiting_subpop]
                        rate_change_compartment *= parameters[
                            transitions[transition_rate_col][transition_index]
                        ][today][visiting_subpop]
    ...
    for time_index, time in tqdm.tqdm(
        enumerate(times), disable=silent
    ):  # , total=len(times)
        ...
        if is_a_new_day:
            ...
            for seeding_instance_idx in range(
                ...
                states_next[seeding_sources][seeding_subpops] = states_next[
                    seeding_sources
                ][seeding_subpops] * (states_next[seeding_sources][seeding_subpops] > 0)
