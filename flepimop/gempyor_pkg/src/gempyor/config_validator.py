...
class SeedingConfig(BaseModel):
    ...
    ] = "NoSeeding"  # note: removed NegativeBinomialDistributed because no longer supported
    ...
    def validate_seedingfile(cls, values):
        ...
        if method == 'PoissonDistributed' and not lambda_file:
            raise ValueError("A `lambda_file` is required when method is 'PoissonDistributed'.")
        if method == 'FolderDraw' and not seeding_file_type:
            raise ValueError("A `seeding_file_type` is required when method is 'FolderDraw'.")
...
class SeirParameterConfig(BaseParameterConfig):
    value: Optional[ValueConfig] = None
    stacked_modifier_method: Annotated[
        str,
        AfterValidator(
            partial(allowed_values, values=["sum", "product", "reduction_product"])
        ),
    ] = None
...
class SeirConfig(BaseModel):
    ...
    parameters: Dict[
        str, SeirParameterConfig
    ]  # there was a previous issue that gempyor doesn't work if there are no parameters (eg if just numbers are used in the transitions) - do we want to get around this?
...
class SourceConfig(
    BaseModel
):  # set up only for incidence or prevalence. Can this be any name? i don't think so atm
...
class OutcomesConfig(BaseModel):
    method: Literal[
        "delayframe"
    ]  # Is this required? I don't see it anywhere in the gempyor code
...
class LikelihoodConfig(BaseModel):
    dist: Annotated[
        ...
        AfterValidator(
            partial(
                allowed_values,
                values=["pois", "norm", "norm_cov", "nbinom", "rmse", "absolute_error"],
            )
        ),
    ] = None
...
class InferenceConfig(BaseModel):
    method: Annotated[
        str,
        AfterValidator(partial(allowed_values, values=["emcee", "default", "classical"])),
    ] = "default"  # for now - i can only see emcee as an option here, otherwise ignored in classical - need to add these options
    iterations_per_slot: Optional[
        int
    ]  # i think this is optional because it is also set in command line??
    do_inference: bool
    gt_data_path: str
