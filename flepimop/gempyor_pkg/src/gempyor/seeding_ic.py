...
class SeedingAndIC:
    ...
    def draw_ic(self, sim_id: int, setup) -> np.ndarray:
        ...
        for pl_idx, pl in enumerate(setup.spatset.subpop_names):
            ...
            if abs(n_y0-n_pop) > 1:
                error = True
                print(f"ERROR: subpop_names {pl} (idx: pl_idx) has a population from initial condition of {n_y0} while population from geodata is {n_pop} (absolute difference should be < 1, here is {abs(n_y0-n_pop)})") 
        if error:
            raise ValueError()
        return y0
