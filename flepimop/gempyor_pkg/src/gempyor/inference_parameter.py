...
class InferenceParameters:
    ...
    def __init__(self, global_config: ConfigView, subpop_names: list[str]) -> None:
        ...
        self._lower_bounds: npt.NDArray[np.float64] = np.array([], dtype=np.float64)
        self._upper_bounds: npt.NDArray[np.float64] = np.array([], dtype=np.float64)
    ...
    def add_modifier(
        ...
        if parameter_config["method"].get() == "SinglePeriodModifier":
            ...
            affected_subpops = list(affected_subpops_grp)
            spatial_groups = []
            for grp_config in parameter_config["groups"]:
                if grp_config["subpop"].get() == "all":
                    affected_subpops_grp = affected_subpops
    ...
    def add_single_parameter(
        ...
        lower, upper = pdist.support
        self._lower_bounds = np.append(self._lower_bounds, lower)
        self._upper_bounds = np.append(self._upper_bounds, upper)
    ...
    def check_in_bound(self, proposal: npt.NDArray[np.float64]) -> bool:
        ...
        return np.logical_and(
            np.greater_equal(proposal, self._lower_bounds),
            np.less_equal(proposal, self._upper_bounds),
        ).all()
    ...
    def inject_proposal(
        ...
        Args:
            proposal: The proposed parameter values.
            snpi_df: A dataframe representing the SEIR modifiers.
            hnpi_df: A dataframe representing the outcome modifiers.
