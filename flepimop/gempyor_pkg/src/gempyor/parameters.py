            ...
            for idx, pn in enumerate(self.pnames):
                ...
                # Parameter given as a file
                    ...
                    if len(df.columns) >= len(nodenames):  # one ts per subpop
                        df = df[nodenames]  # make sure the order of subpop is the same as the reference
                        ...
                        columns are {len(df.columns)}, expected {len(nodenames)} (the number of subpop) or one."""
                        )
