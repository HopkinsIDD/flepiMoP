...
imm_esc_start <- config$start_date #"2022-01-01"
imm_esc_months <- as.numeric((lubridate::as_date(config$start_date) - lubridate::as_date(imm_esc_start))) / 30  # months since omicron # average of 2 assuming beginning of Jan 2022 peak.
...
seir_dat <- seir_dat %>%
    ...
    # ungroup() %>%
    filter(!(mc_variant_type %in% c("VARIANTX")))
...
seir_dat_static <- seir_dat %>%
    ...
    filter(!(mc_variant_type %in% c("DELTA")))
...
seir_dat_changing %>% filter(mc_age_strata == "age18to64") %>%
    left_join(geodata %>% rename(loc = subpop) %>% select(USPS, loc, mc_age_strata = age_strata, pop_agestrata) %>% distinct()) %>%
    mutate(prop = n / pop_agestrata) %>%
    # group_by(date, mc_age_strata, USPS) %>%
    # summarise(prop_imm
...
seir_dat_changing %>%
    left_join(geodata %>% rename(loc = subpop) %>% select(USPS, loc, mc_age_strata = age_strata, pop_agestrata) %>% distinct()) %>%
    mutate(prop = n / pop_agestrata) %>%
    group_by(USPS, date, mc_value_type, mc_vaccination_stage, mc_age_strata) %>%
    summarise(prop_immune = sum(n * prob_immune_nom, na.rm = TRUE) / sum(n, na.rm = TRUE)) %>%
    ggplot(aes(x = USPS, y = prop_immune, fill = USPS)) +
    geom_bar(stat = "identity") +
    theme(legend.position = "none", axis.text.x = element_text(angle = 90)) +
    theme_bw() +
    facet_grid(mc_age_strata~mc_vaccination_stage)
...
#     mutate(prop = n / pop_agestrata) %>%
#     mutate(mc_infection_stage = factor(mc_infection_stage, levels = paste0("X", 0:10))) %>%
#     ggplot(aes(x = mc_infection_stage, y = prop, color = mc_vaccination_stage)) +
#     geom_point() +
#     facet_wrap(~USPS, nrow = 9, scales = "free_y") +
#     theme_bw() +
#     ylab("Propotion of Population") +
