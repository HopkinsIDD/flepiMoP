...
With this method users can specify arbitrary initial conditions in a convenient formatted input CSV or parquet file. For example, for a model with the following `compartments` and `initial_conditions` sections:
...
Furthermore, users can also request timeseries parameters as well and these will be provided as a numpy array with shape `(# days, # subpopulations)`.
