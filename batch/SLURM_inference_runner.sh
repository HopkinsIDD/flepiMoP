...
if [ -n "$LAST_JOB_OUTPUT" ]; then  # -n Checks if the length of a string is nonzero --> if LAST_JOB_OUTPUT is not empty, the we download the output from the last job
	...
	do
		...
		do
			export OUT_FILENAME=$(python -c "from gempyor import file_paths; print(file_paths.create_file_name(run_id='$FLEPI_RUN_INDEX',prefix='$FLEPI_PREFIX/$FLEPI_RUN_INDEX/$liketype/intermediate/%09d.'% $FLEPI_SLOT_INDEX,index=$FLEPI_BLOCK_INDEX-1,ftype='$filetype',extension='$extension', model_output_path='$FLEPI_MODEL_OUTPUT_PATH'))")
			if [ $FLEPI_BLOCK_INDEX -eq 1 ]; then
				export IN_FILENAME=$(python -c "from gempyor import file_paths; print(file_paths.create_file_name(run_id='$RESUME_FLEPI_RUN_INDEX',prefix='$FLEPI_PREFIX/$RESUME_FLEPI_RUN_INDEX/$liketype/final/',index=$FLEPI_SLOT_INDEX,ftype='$filetype',extension='$extension', model_output_path='$FLEPI_MODEL_OUTPUT_PATH'))")
			else
	...
	ls -ltr $FLEPI_MODEL_OUTPUT_PATH
fi
...
if [ $S3_UPLOAD == "true" ]; then
    ...
    do
        export FILENAME=$(python -c "from gempyor import file_paths; print(file_paths.create_file_name(run_id='$FLEPI_RUN_INDEX',prefix='$FLEPI_PREFIX/$FLEPI_RUN_INDEX/chimeric/intermediate/%09d.'% $FLEPI_SLOT_INDEX, index=$FLEPI_BLOCK_INDEX,ftype='$type',extension='parquet', model_output_path='$FLEPI_MODEL_OUTPUT_PATH'))")
        aws s3 cp --quiet $FILENAME $S3_RESULTS_PATH/$FILENAME
    done
        for type in "seed"
        do
            export FILENAME=$(python -c "from gempyor import file_paths; print(file_paths.create_file_name(run_id='$FLEPI_RUN_INDEX',prefix='$FLEPI_PREFIX/$FLEPI_RUN_INDEX/chimeric/intermediate/%09d.'% $FLEPI_SLOT_INDEX, index=$FLEPI_BLOCK_INDEX,ftype='$type',extension='csv', model_output_path='$FLEPI_MODEL_OUTPUT_PATH'))")
        aws s3 cp --quiet $FILENAME $S3_RESULTS_PATH/$FILENAME
    done
        for type in "seed"
        do
    ...
    do
        ...
        for type in "seed"
    do
        export FILENAME=$(python -c "from gempyor import file_paths; print(file_paths.create_file_name(run_id='$FLEPI_RUN_INDEX',prefix='$FLEPI_PREFIX/$FLEPI_RUN_INDEX/global/final/', index=$FLEPI_SLOT_INDEX,ftype='$type',extension='csv', model_output_path='$FLEPI_MODEL_OUTPUT_PATH'))")
        aws s3 cp --quiet $FILENAME $S3_RESULTS_PATH/$FILENAME
    done
fi
