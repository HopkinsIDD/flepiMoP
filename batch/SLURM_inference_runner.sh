...
echo "Rscript $FLEPI_PATH/flepimop/main_scripts/inference_slot.R --config $CONFIG_PATH   # path to the config file
                                               ...
                                               --scenarios $FLEPI_SCENARIOS  # name of the intervention to run, or 'all'
                                               --deathrates $FLEPI_DEATHRATES  # name of the outcome scenarios to run, or 'all'
...
Rscript $FLEPI_PATH/flepimop/main_scripts/inference_slot.R -p $FLEPI_PATH --this_slot $FLEPI_SLOT_INDEX --config $CONFIG_PATH --run_id $FLEPI_RUN_INDEX --scenarios $FLEPI_SCENARIOS --deathrates $FLEPI_DEATHRATES --jobs 1 --iterations_per_slot $FLEPI_ITERATIONS_PER_SLOT --this_block 1 --stoch_traj_flag $FLEPI_STOCHASTIC_RUN --is-resume $RESUME_RUN --is-interactive FALSE > $LOG_FILE 2>&1
dvc_ret=$?
