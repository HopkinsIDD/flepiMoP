...
Rscript $FLEPI_PATH/build/local_install.R
local_install_ret=$?
...
(cd $FLEPI_PATH && pip install -e flepimop/gempyor_pkg)
python_install_ret=$?
...
Rscript $FLEPI_PATH/flepimop/main_scripts/inference_slot.R -p $FLEPI_PATH
dvc_ret=$?
